...
const translations = reactive({
  en: {
    ...
    resourceAlertTip: 'Add rules to enable resource load alerts; removing all rules disables them. Overlapping rules for the same server and monitor item are notified separately. DO keeps one sample per server per minute, up to 10 minutes.',
  ...
  zh: {
    ...
    resourceAlertTip: '添加规则表示开启资源负载告警；删除全部规则表示关闭。同一服务器同一监控项配置多条规则会分别触发通知。DO 按每台服务器每分钟缓存一个样本，最多保留 10 分钟。',
