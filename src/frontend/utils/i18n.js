...
const translations = reactive({
  en: {
    ...
    trafficAlertPercentTip: 'Per-server threshold percentage; 0 = follow global setting. Sends one notification per billing cycle; re-arms after traffic reset or when threshold/limit changes. Only applies when a traffic limit is set.',
    trafficAlertThreshold: 'Monthly Traffic Alert Threshold',
    trafficAlertThresholdTip: 'Global default: notify once when a server\'s monthly traffic reaches this percentage of its traffic limit. 0 = off. Servers with their own threshold override this.',
  ...
  zh: {
    ...
    trafficAlertPercentTip: '逐台阈值百分比；0=跟随全局设置。每账期仅通知一次，流量重置或阈值/限额变更后重新可触发。仅在设置了流量限制时生效。',
    trafficAlertThreshold: '月流量告警阈值',
    trafficAlertThresholdTip: '全局默认：当某服务器当月流量达到其流量限制的该百分比时通知一次；0=关闭。设置了逐台阈值的服务器优先于全局。',
